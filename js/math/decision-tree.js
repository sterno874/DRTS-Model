...
export function computeDecisionTree(p) {
  ...
  const fda = normalizeBranchWeights(
    p.pAcceptModularPct ?? 65,
    p.pDeferDataPct ?? 25,
    p.pRejectPct ?? 10
  );
  const comm = normalizeBranchWeights(
    p.pCommFastPct ?? 25,
    p.pCommBasePct ?? 50,
    p.pCommSlowPct ?? 25
  );
  ...
  const shares = val.v_shares ?? 88;
  const cash = val.v_cash ?? 0;

  /** @type {Array<{ id: string, label: string, prob: number, ev: number, perSh: number, tag?: string }>} */
  const paths = [];
  ...
  const weightedPerSh = evPerShare(weightedEv, shares, cash);
