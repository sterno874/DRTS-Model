...
<p><span class="tag m">Model</span> ReSTART ORR slider priors span ~45–65% (1σ) as undisclosed assumptions. Benchmark slider anchors cemiplimab 47% (◆) and literature floor 26–40%. Wilson 95% CI brackets binomial uncertainty at n=88. DOR is a pass/fail gate in this model, not a full Kaplan–Meier survival curve — if responses do not last 6 months, the co-primary fails even if ORR looks strong.</p>
<h4>PMA vs NDA</h4>
...
<p><span class="tag m">Model</span> Monte Carlo: true ORR ~ Beta(20p, 20(1−p)) centered on assumed slider; simulate n=88 binomial outcomes; success if ORR ≥ threshold (default 35%, SAP not public) AND beats benchmark. Seeded n=1500 for reproducibility. DOR modeled as pass/fail vs 6-month gate — not Kaplan–Meier.</p>
<h4>PMA vs NDA</h4>
...
<p><span class="tag f">Fact</span> ReSTART (n=88) co-primaries ORR + 6-month DOR are the registrational spine for US skin. ORR inference here uses Wilson CI + exact binomial vs historical p₀; DOR is a binary gate in-model, not KM/Brookmeyer–Crowley. SAP-defined thresholds are not public — default success threshold 35% is an assumption. Secondary PFS/OS at 1 yr are supportive readouts, not co-primary gates.</p>
<p><span class="tag m">Model</span> Combined P(PMA) blends a structural ORR/DOR score (55%) with a subjective PMA prior slider (45%). Monte Carlo draws true ORR from a moderate Beta prior, simulates n=88 binomial outcomes, and scores success only if both the ORR threshold and historical-benchmark beat hold. Seeded n=1500 for reproducible histograms across share links.</p>
...
<p><span class="tag u">Assumption</span> Per-share outputs divide risk-adjusted EV by ~42M ADS. Ordinary shares outstanding (88,009,737 per F-3) and ADS ratio matter for fully diluted paths. F-3 shelf up to $300M + $100M ATM (<a href="https://www.sec.gov/Archives/edgar/data/1871321/000121390026048160/ea0285710-f3_alpha.htm" target="_blank" rel="noopener">SEC F-3 Apr 2026</a>) is capacity, not issuance — the model does not auto-dilute on shelf size. Stress-test $/share against higher share counts before treating any scenario as a price target.</p>
